...
namespace can_utils
{
  ...
  template <typename T>
  static can_plugins::Frame makeFrame(const uint16_t id, const T data) //シラスのid,data??
  {
    can_plugins::Frame frame;
    // when you want to use SetTarget, you need to set the id to the following id.
    frame.id = id + 1; //シラスのBID+1(target)専用。
  ...
  // BaseBoardのwrapper.cppに定義されている。
  namespace BaseBoardCommand
  {
    enum Command : std::uint8_t
    {
      frot,    //=0, foword_rotation 正転
      brot,    //=1, back_rotation 逆転
      disable, //=2
      enable,  //=3
  ...
  inline can_plugins::Frame makeFrame(const uint16_t id, Command cmd)
  {
    // when you use the id, it works as the command if moter.
    return makeFrame(id - 1, (uint8_t)cmd); //-1に変更。プラスマイナスゼロ。
  }
  inline can_plugins::Frame makeFrame(const uint16_t id, BaseBoardCommand::Command cmd)
  {
    // when you use the id, it works as the command if moter.
    return makeFrame(id - 1, (uint8_t)cmd); //-1に変更。プラスマイナスゼロ。
  }
